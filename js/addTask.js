...
async function createSubtask() {
    ...
    if (subtaskText !== '') {
        let newSubtask = new Subtask(undefined, subtaskText)
        
        let response = await createASubtask(subtaskText)
        let responseAsJson = await response.json()
        subtaskIDsTemp.push(responseAsJson.id)
 ...
 * @param {number} m - The index of the subtask to be changed.
 */
async function fixSubtasks(m) {
    let subtaskId = subtaskTemp[m].id    
    let containerTodoSubtask = `containerTodoSubtask${m}`;
    let fixTodoSubtask = `fixTodoSubtask${m}`;
    ...
    updateJson = JSON.stringify({"text": editFixSubtask.value})
    await patchASubtask(updateJson, subtaskId)
 ...
 */
async function deleteSubtask(m) {
    subtaskId = subtaskTemp[m].id
    
    await deleteASubtask(subtaskId)
    subtaskIDsTemp = subtaskIDsTemp.filter(id => id !== subtaskId)
    subtaskTemp.splice(m, 1)
    renderSubtasks();
}


async function pushSubtask(subtask){
    return await fetch()
}

async function serializeTask(task){
    let subtasksToAdd = await allSubtasksToJson()
    // let subtasks = () =>{
    //     json = []
    ...
    // }
    return {
        "worker_ids": task.worker,
        "title": task.title,
        "description": task.desc,
        "date": task.date,
        "subtask_ids": subtaskIDsTemp,
        "prio": task.prio,
